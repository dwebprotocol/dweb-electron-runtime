...
module.exports = function (protocol) {
  const { network, basestore } = new Client()
  const active = new Map()

  function getDrive (basestore, key) {
    ...
    a = { refs: 1, drive: ddrive(basestore(), Buffer.from(key, 'hex')), done }
    a.drive.ready(() => {
  ...
  protocol.registerStreamProtocol('cjs', (request, callback) => {
    ...
    const d = getDrive(basestore, query.key)
  ...
  protocol.registerStreamProtocol('dweb', (request, callback) => {
    const u = request.url.replace('dweb://', '').split('?')[0]
    ...
    const d = getDrive(basestore, key)
