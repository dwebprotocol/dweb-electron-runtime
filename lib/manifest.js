...
module.exports = class Manifest {
  constructor (drives, url) {
    ...
    this.drives = drives
    this.url = url
    this.isDDrive = p.protocol === 'dweb:'
    this.drive = this.isDDrive ? drives.checkout(p.hostname) : fs
  ...
  async load () {
    ...
    await new Promise((resolve, reject) => {
      this.drive.readFile(this.path + '/dhub.json', (err, buf) => {
        if (err) return reject(err)
